    ...
    sealed class CDeserializeProxy : Serde.IDeserialize<C>
    {
        C Serde.IDeserialize<C>.Deserialize(IDeserializer deserializer)
        {
            ...
            while ((_l_index_ = typeDeserialize.TryReadIndex(_l_serdeInfo, out _)) != IDeserializeType.EndOfType)
            {
                ...
                {
                    case 0:
                        _l_colorint = typeDeserialize.ReadValue<ColorInt, ColorIntProxy>(_l_index_);
                        _r_assignedValid |= ((byte)1) << 0;
                        break;
                    case 1:
                        _l_colorbyte = typeDeserialize.ReadValue<ColorByte, ColorByteProxy>(_l_index_);
                        _r_assignedValid |= ((byte)1) << 1;
                        break;
                    case 2:
                        _l_colorlong = typeDeserialize.ReadValue<ColorLong, ColorLongProxy>(_l_index_);
                        _r_assignedValid |= ((byte)1) << 2;
                        break;
                    case 3:
                        _l_colorulong = typeDeserialize.ReadValue<ColorULong, ColorULongProxy>(_l_index_);
