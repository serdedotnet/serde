...
namespace Serde.Test
{
    public class JsonValueFsCheck
    {
        ...
        {
            JsonValueGenerators.GenJsonValue.Sample(
                item =>
                {
    ...
    {
        public static Gen<JsonValue> GenPrimitive { get; } =
            Gen.OneOf<JsonValue>(
                Gen.Int.Select(a => (JsonValue)new Number(a)),
                Gen.Double.Where(d => double.IsFinite(d) && !double.IsNaN(d))
                    .Select(a => (JsonValue)new Number(a))
            );

        public static Gen<JsonValue> GenJsonValue { get; } =
            Gen.Recursive<JsonValue>(
                (depth, self) =>
                {
                    if (depth >= 4)
                        return GenPrimitive;

                    var genArray = Gen.Int[1, 3]
                        .SelectMany(n =>
                            self.Array[n]
                                .Select(values => (JsonValue)new Array(values.ToImmutableArray()))
                        );

                    var genObject = Gen.Int[1, 3]
                        .SelectMany(n =>
                            self.Array[n]
                                .Select(values =>
                                {
