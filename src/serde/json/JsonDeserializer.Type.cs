...
partial class JsonDeserializer<TReader> : ITypeDeserializer
{
    ...
    Task<int> ITypeDeserializer.TryReadIndex(ISerdeInfo serdeInfo, out string? errorName)
    {
        ...
        else
        {
            ...
            if (peek == (byte)'}')
            {
                errorName = null;
                Reader.Advance();
                return Task.FromResult(ITypeDeserializer.EndOfType);
            }
            if (!_first)
            {
                if (peek != (byte)',')
        ...
        _first = false;
        return Task.FromResult(localIndex);
    }
