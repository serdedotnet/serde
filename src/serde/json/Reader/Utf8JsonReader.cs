...
namespace System.Text.Json
{
    ...
    public partial struct Utf8JsonReader
    {
        ...
        internal ReadOnlySpan<byte> OriginalSpan => _sequence.IsEmpty ? GetBuffer : default;
        ...
        public ReadOnlySpan<byte> ValueSpan => _valueMemory.Span;
        ...
        ///   </para>
        /// </remarks>
        public Utf8JsonReader(ReadOnlyMemory<byte> jsonData, JsonReaderOptions options = default)
        ...
        private bool ConsumeString()
        {
            ...
            if (idx >= 0)
            {
                ...
                {
                    return ConsumeStringAndValidate(mem, idx);
                }
            }
            else
            {
        ...
        private bool ConsumeStringAndValidate(ReadOnlyMemory<byte> memData, int idx)
        {
        ...
        private bool TryGetNumber(ReadOnlyMemory<byte> memData, out int consumed)
        {
            var data = memData.Span;
            ...
            ConsumeNumberResult signResult = ConsumeNegativeSign(data, ref i);
            ...
            if (nextByte == '0')
            {
                ConsumeNumberResult result = ConsumeZero(data, ref i);
            ...
            {
                i++;
                ConsumeNumberResult result = ConsumeIntegerDigits(data, ref i);
                if (result == ConsumeNumberResult.NeedMoreData)
                {
                    return false;
                }
            ...
            signResult = ConsumeSign(data, ref i);
            if (signResult == ConsumeNumberResult.NeedMoreData)
            {
                return false;
            }
            ...
            i++;
            ConsumeNumberResult resultExponent = ConsumeIntegerDigits(data, ref i);
        ...
        private ConsumeNumberResult ConsumeNegativeSign(ReadOnlySpan<byte> data, scoped ref int i)
        {
            byte nextByte = data[i];
        ...
        private ConsumeNumberResult ConsumeZero(ReadOnlySpan<byte> data, scoped ref int i)
        {
        ...
        private ConsumeNumberResult ConsumeIntegerDigits(ReadOnlySpan<byte> data, scoped ref int i)
        {
        ...
        private ConsumeNumberResult ConsumeDecimalDigits(ref ReadOnlySpan<byte> data, scoped ref int i)
        {
            ...
            return ConsumeIntegerDigits(data, ref i);
        }

        private ConsumeNumberResult ConsumeSign(ReadOnlySpan<byte> data, scoped ref int i)
        {
            if (i >= data.Length)
            {
                if (IsLastSpan)
        ...
        {
            // Create local copy to avoid bounds checks.
            ReadOnlySpan<byte> localBuffer = GetBuffer.Slice(_consumed + 1);
