    ...
    public partial struct Utf8JsonReader
    {
        ...
        public Utf8JsonReader(ReadOnlySequence<byte> jsonData, bool isFinalBlock, JsonReaderState state)
        {
            ...
            else
            {
                ...
                bool firstSegmentIsEmpty = GetBuffer.Length == 0;
        ...
        private bool ReadMultiSegment()
        {
            ...
            byte first = GetBuffer[_consumed];
        ...
        {
            if (_consumed >= (uint)GetBuffer.Length)
            {
        ...
        {
            var mem = _buffer.Slice(_consumed);
            ...
            _valueMemory = mem.Slice(0, literal.Length);
            HasValueSequence = false;
        Done:
        ...
        private bool ConsumeNumberMultiSegment()
        {
            ...
            if (_consumed >= (uint)GetBuffer.Length)
            {
                ...
                if (_isNotPrimitive)
                {
                    ThrowHelper.ThrowJsonReaderException(ref this, ExceptionResource.ExpectedEndOfDigitNotFound, GetBuffer[_consumed - 1]);
                }
            }
            ...
            Debug.Assert(
                ((_consumed < GetBuffer.Length) &&
                !_isNotPrimitive &&
                JsonConstants.Delimiters.IndexOf(GetBuffer[_consumed]) >= 0)
                || (_isNotPrimitive ^ (_consumed >= (uint)GetBuffer.Length)));
        ...
        {
            Debug.Assert(GetBuffer.Length >= _consumed + 1);
            Debug.Assert(GetBuffer[_consumed] == JsonConstants.Quote);

            // Create local copy to avoid bounds checks.
            var mem = _buffer.Slice(_consumed + 1);
            ReadOnlySpan<byte> localBuffer = mem.Span;
            ...
            if (idx >= 0)
            {
                ...
                {
                    return ConsumeStringAndValidateMultiSegment(mem, idx);
                }
            }
            else
            {
        ...
        private bool ConsumeStringNextSegment()
        {
            ...
            HasValueSequence = true;
            int leftOver = GetBuffer.Length - _consumed;
            ...
            while (true)
            {
                ...
                //Create local copy to avoid bounds checks.
                ReadOnlySpan<byte> localBuffer = GetBuffer;
                int idx = localBuffer.IndexOfQuoteOrAnyControlOrBackSlash();
        ...
        private bool ConsumeStringAndValidateMultiSegment(ReadOnlyMemory<byte> memData, int idx)
        {
            var data = memData.Span;
            ...
            SequencePosition end;
            HasValueSequence = false;
            int leftOverFromConsumed = GetBuffer.Length - _consumed;
        ...
        Done:
            ...
            else
            {
                ...
                _consumed += idx + 2;
                _valueMemory = memData.Slice(0, idx);
            }
        ...
        private bool TryGetNumberMultiSegment(ReadOnlyMemory<byte> memData, out int consumed)
        {
            var data = memData.Span;
            ...
            ConsumeNumberResult signResult = ConsumeNegativeSignMultiSegment(data, ref i, rollBackState);
            ...
            if (nextByte == '0')
            {
                ConsumeNumberResult result = ConsumeZeroMultiSegment(data, ref i, rollBackState);
            ...
            {
                ConsumeNumberResult result = ConsumeIntegerDigitsMultiSegment(data, ref i);
                if (result == ConsumeNumberResult.NeedMoreData)
                {
                    RollBackState(rollBackState);
                    return false;
            ...
            if (nextByte == '.')
            {
                i++;
                _bytePositionInLine++;
                ConsumeNumberResult result = ConsumeDecimalDigitsMultiSegment(data, ref i, rollBackState);
                if (result == ConsumeNumberResult.NeedMoreData)
                {
                    RollBackState(rollBackState);
                    return false;
            ...
            signResult = ConsumeSignMultiSegment(data, ref i, rollBackState);
            if (signResult == ConsumeNumberResult.NeedMoreData)
            {
                RollBackState(rollBackState);
                return false;
            ...
            i++;
            _bytePositionInLine++;
            ConsumeNumberResult resultExponent = ConsumeIntegerDigitsMultiSegment(data, ref i);
        ...
        Done:
            ...
            {
                _valueMemory = memData.Slice(0, i);
                consumed = i;
            }
            return true;
        }

        private ConsumeNumberResult ConsumeNegativeSignMultiSegment(ReadOnlySpan<byte> data, scoped ref int i, scoped in PartialStateForRollback rollBackState)
        {
            Debug.Assert(i == 0);
            byte nextByte = data[i];
        ...
        private ConsumeNumberResult ConsumeZeroMultiSegment(ReadOnlySpan<byte> data, scoped ref int i, scoped in PartialStateForRollback rollBackState)
        {
        ...
        private ConsumeNumberResult ConsumeIntegerDigitsMultiSegment(ReadOnlySpan<byte> data, scoped ref int i)
        {
        ...
        private ConsumeNumberResult ConsumeDecimalDigitsMultiSegment(ReadOnlySpan<byte> data, scoped ref int i, scoped in PartialStateForRollback rollBackState)
        {
            if (i >= data.Length)
            {
                if (IsLastSpan)
            ...
            i++;
            _bytePositionInLine++;
            return ConsumeIntegerDigitsMultiSegment(data, ref i);
        }

        private ConsumeNumberResult ConsumeSignMultiSegment(ReadOnlySpan<byte> data, scoped ref int i, scoped in PartialStateForRollback rollBackState)
        {
            if (i >= data.Length)
            {
                if (IsLastSpan)
        ...
        private bool SkipCommentMultiSegment(out int tailBytesToIgnore)
        {
            ...
            _bytePositionInLine++;
            // Create local copy to avoid bounds checks.
            ReadOnlySpan<byte> localBuffer = GetBuffer.Slice(_consumed);
